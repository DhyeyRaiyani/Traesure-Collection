...
function draw() {
  ...
  if (rubyGroup.isTouching(boy)){
    rubyGroup.destroyEach();
    treasure = treasure+8;
  }
  
  
  path.velocityY = (4 + 2*treasure/20);
...
function createCash() {
  ...
  cash.scale=0.12;
  cash.velocityY = (4 + 2*treasure/20);
  cash.lifetime = 150;
  cashG.add(cash);
  }
}
...
function createDiamonds() {
  ...
  diamonds.scale=0.03;
  diamonds.velocityY = (4 + 2*treasure/20);
...
function createJwellery() {
  ...
  jwellery.scale=0.13;
  jwellery.velocityY = (4 + 2*treasure/20);
...
function createSword(){
  ...
  sword.scale=0.1;
  sword.velocityY = (4 + 2*treasure/20);
  sword.lifetime = 150;
...
if (World.frameCount % 180 == 0){
  ...
  ruby.addImage("ruby", rubyImage);
  ruby.scale = 0.05;
  ruby.velocityY = (4 + 2*treasure/20);
  ruby.lifetime = 150;
  rubyGroup.add(ruby);
}
